...
import { SeventhProtocol } from "../typechain-types";
import { SignerWithAddress } from "@nomicfoundation/hardhat-ethers/signers";
import { time } from "@nomicfoundation/hardhat-network-helpers";

describe("SeventhProtocol", function () {
    let seventhProtocol: SeventhProtocol;
    ...
    beforeEach(async function () {
        ...
        // Deploy the contract
        const SeventhProtocolFactory = await ethers.getContractFactory("SeventhProtocol");
        seventhProtocol = await SeventhProtocolFactory.deploy();
    ...
    describe("Deployment", function () {
        it("should deploy successfully", async function () {
            expect(await seventhProtocol.getAddress()).to.be.properAddress;
        });
    });
    ...
    describe("SetTuple", function () {
        it("should allow setting a tuple with future timestamp", async function () {
            ...
            const tx = await seventhProtocol.connect(addr1).SetTuple(
            ...
            const [sig, t, k] = await seventhProtocol.GetTuple(testPid);
        ...
        it("should reject setting tuple with past timestamp", async function () {
            ...
            await expect(
                seventhProtocol.connect(addr1).SetTuple(testSig, pastTime, testKey, testPid)
            ).to.be.revertedWithCustomError(seventhProtocol, "TimestampMustBeFuture");
        });
        ...
        it("should reject setting tuple with current timestamp", async function () {
            ...
            await expect(
                seventhProtocol.connect(addr1).SetTuple(testSig, uint48(currentTimestamp), testKey, testPid)
            ).to.be.revertedWithCustomError(seventhProtocol, "TimestampMustBeFuture");
        });
        ...
        it("should reject setting duplicate entries", async function () {
            ...
            await seventhProtocol.connect(addr1).SetTuple(testSig, futureTime, testKey, testPid);
            
            // Second call should fail
            await expect(
                seventhProtocol.connect(addr1).SetTuple(testSig, futureTime, testKey, testPid)
            ).to.be.revertedWithCustomError(seventhProtocol, "EntryAlreadyExists");
        });
        ...
        it("should allow different users to set different tuples", async function () {
            ...
            const tx1 = await seventhProtocol.connect(addr1).SetTuple(
                testSig, futureTime, testKey, testPid
            );
            const receipt1 = await tx1.wait();
            
            // Second user sets their tuple
            const tx2 = await seventhProtocol.connect(addr2).SetTuple(
            ...
            // Verify both tuples
            const [sig1, t1, k1] = await seventhProtocol.GetTuple(testPid);
            expect(sig1).to.equal(testSig);
            expect(t1).to.equal(futureTime);
            expect(k1).to.equal(testKey);
            
            const [sig2, t2, k2] = await seventhProtocol.GetTuple(testPid2);
        ...
        it("should measure gas used for SetTuple with different signature lengths", async function () {
            ...
            const tx2 = await seventhProtocol.connect(addr1).SetTuple(
    ...
    describe("GetTuple", function () {
        it("should retrieve a previously set tuple", async function () {
            ...
            const gasEstimate = await seventhProtocol.GetTuple.estimateGas(testPid);
        ...
        it("should reject retrieving non-existent tuple", async function () {
            ...
            await expect(
                seventhProtocol.GetTuple(nonExistentPid)
            ).to.be.revertedWithCustomError(seventhProtocol, "EntryDoesNotExist");
        });
    });
    ...
    describe("Gas usage comparison", function() {
        it("should measure gas usage with different sizes of data", async function() {
            ...
            const tx3 = await seventhProtocol.connect(addr1).SetTuple(
        ...
        it("should provide a complete gas usage report", async function() {
            ...
            const SeventhProtocolFactory = await ethers.getContractFactory("SeventhProtocol");
            const deployTx = await SeventhProtocolFactory.getDeployTransaction();
            ...
            const setTx = await seventhProtocol.connect(addr1).SetTuple(
                testSig, futureTime, testKey, testPid
            );
            ...
            const getTupleEstimate = await seventhProtocol.GetTuple.estimateGas(testPid);
            ...
            const emptyContractFactory = await ethers.getContractFactory("SeventhProtocol");
            ...
            for (let i = 0; i < 10; i++) {
                ...
                await seventhProtocol.connect(addr1).SetTuple(
            ...
            const filledStorageTx = await seventhProtocol.connect(addr1).SetTuple(
